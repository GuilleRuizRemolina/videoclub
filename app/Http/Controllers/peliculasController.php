...
class peliculasController extends Controller
{
    ...
    //funcion encargada de recoger los datos y editar dicho usuario
    ...
    //funcion que filtra la lista de peliculas por su nombre
    public function buscarPelicula(Request $request)
    {
        $nombreBuscar = $request->buscar;
        ...
        foreach($peliculas as $p)
        {
            //la funcion substr() compara un string con otro, desde la posiccion 0 hasta la longitud de nombreBuscar
            //para calcular la longitud de nombreBuscar use la funcion srtlen()
            //para que las mayusculas y minusculas no dieran problemas converti todo a minusculas con srttolower()
            if(substr(strtolower($p->titulo), 0,strlen($nombreBuscar)) == strtolower($nombreBuscar))
            {
                array_push($peliculasFiltradas,$p);
            }
        }

        return view('listaPeliculasFiltradas',['listado' => $peliculasFiltradas]);
